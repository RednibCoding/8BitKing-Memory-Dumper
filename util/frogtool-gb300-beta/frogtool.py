...
def convert_zip_image_to_zxx(path, img_file, zip_file, zxx_ext):
    ...
    converted = argb8888_convert(img_file_path, zxx_file_path, (144, 208))
    if not converted:
        return False
...
def rgb888_convert(src_filename, dest_filename, dest_size=None):
    try:
        srcimage = Image.open(src_filename)
    except (OSError, IOError):
        print(f"! Failed opening image file {src_filename} for conversion")
    ...
    for h in range(image_height):
        for w in range(image_width):
            r, g, b = pixels[w, h]
            # No need to shift bits as each component is already 8 bits
            dest_file.write(struct.pack('BBB', r, g, b))

    dest_file.close()
    return True

def argb8888_convert(src_filename, dest_filename, dest_size=None):
    try:
        srcimage = Image.open(src_filename)
    except (OSError, IOError):
        print(f"! Failed opening image file {src_filename} for conversion")
    ...
    for y in range(image_height):
        for x in range(image_width):
            ...
            dest_file.write(struct.pack('BBBB', a, r, g, b))
